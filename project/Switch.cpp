...
void Switch::Initialize(Vector3 position, Camera* camera, DirectXBase* dxc,Input*input)
{
	directX = dxc;
	switchCamera = camera;
...
void Switch::Draw(Microsoft::WRL::ComPtr<ID3D12Resource>directionalLightResource, Microsoft::WRL::ComPtr<ID3D12Resource>pointLightResource,  Microsoft::WRL::ComPtr<ID3D12Resource>spotLightResource)
{
	if (switchFlag) {
		switchModel->SetModel("box.obj");
